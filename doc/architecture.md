...
- OMP RPC, ACP, model queries, render, HTML export, and doctor probes receive the inherited environment minus `OMP_TELEGRAM_BOT_TOKEN`; explicit OMP configuration variables remain untouched. This prevents accidental child-environment inheritance of the default bridge token, not access to same-user processes or other token copies.
