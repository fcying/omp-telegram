...
- `max_workers` bounds connected OMP processes, not logical sessions. A positive `idle_timeout` may release a quiescent worker's process and return its slot while retaining its validated binding and session claim.
...
Without arguments, `/new` reuses the saved conversation workspace. With no saved workspace, it resolves and uses `workspace_root` itself; it does not create a per-conversation subdirectory. Database read failures still fail instead of falling back. Conversations selecting the same directory share files, not native session identities.
