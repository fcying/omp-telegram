...
Programmatic invalidation uses the same bounded cleanup queue: canceling a resume list, native UI cancellation, closing/replacing an instance, and terminal task completion all remove tokens and enqueue their known menu IDs. This invalidation helper does not send native UI replies; only explicit local cancellation paths do so, and only for the current generation. A native cancel event is not echoed back to OMP. Cleanup remains best-effort and is not guaranteed after worker-context cancellation or queue overflow.
