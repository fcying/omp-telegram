...
On the first `/new` in an unbound forum topic, the bridge synchronously sets the Telegram topic title to the basename of the resolved workspace. Later session replacements do not rename the topic; `/name` changes only the native OMP session title.
